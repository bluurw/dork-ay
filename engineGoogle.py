...
results_num = 10
lang = 'en'
start_index = 0 # indica em qual indice deve comecar
proxies = None # [https://5.111.250.18:856, http://95.111.250.18:1456]
timeout = 5
details = False
safe = 'active' # [active] [off]
ssl_verify = False # [True] [False] [None]
region = None
unique = True # Nao mantem resultados repetidos
'''
...
def search(param, results_num=10, lang='en', start_index=0, proxies=None, timeout=5, details=False, safe='active', ssl_verify=None, region=None, unique=True):
    ...
    while current_results_num < results_num:
        ...
        response = request(param, results_num, lang, start_index, proxies, timeout, safe, ssl_verify, region)
        ...
        for result in result_block:
            ...
            if metadata['url'] == 'Unknown' or not metadata['url'].startswith('http') or not metadata['url'].startswith('https'):
                continue
